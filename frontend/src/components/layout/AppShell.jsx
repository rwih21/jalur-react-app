  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <aside className="fixed inset-y-0 left-0 hidden w-64 border-r bg-white p-5 lg:block">
        ...
        {user && (
          <div className="mt-2 text-xs text-slate-400">Hi, {user.name}</div>
        )}
        ...
        <button
          onClick={logout}
          className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-slate-400 hover:text-red-500 mt-auto absolute bottom-5"
        >
          <LogOut size={18} />
          Logout
        </button>
