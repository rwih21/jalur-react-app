...
const spanner_api_1 = require("@google-cloud/spanner-api");
var spannerClient = spanner_api_1.protos.google;
var google = spanner_api_1.protos.google;
var IsolationLevel = google.spanner.v1.TransactionOptions.IsolationLevel;
var ReadLockMode = google.spanner.v1.TransactionOptions.ReadWrite.ReadLockMode;
 ...
 */
function injectGaxOpt(existingOpts, key, value) {
...
class Snapshot extends events_1.EventEmitter {
        ...
        if (session.metadata && session.metadata.multiplexed) {
            ...
            this.request = (config, callback) => {
                ...
                if (!config.gaxOpts || Object.keys(config.gaxOpts).length === 0) {
                    ...
                    gaxOpts = injectGaxOpt(config.gaxOpts, 'affinityKey', this._affinityKey);
                }
                config = Object.assign({}, config, { gaxOpts });
                return session.request(config, callback);
            };
            this.requestStream = (config) => {
                let gaxOpts;
                if (!config.gaxOpts || Object.keys(config.gaxOpts).length === 0) {
                    gaxOpts = this._bindGaxOpts;
                }
     ...
     */
        ...
        if (this._affinityKey) {
            const database = this.session?.parent;
            const spanner = database?.parent?.parent;
            const client = spanner?.clients_?.get('SpannerClient');
    ...
    run(query, callback) {
        ...
        (0, instrument_1.startTrace)('Snapshot.run', {
            ...
            return this.runStream(query)
                ...
                if (!('code' in err)) {
                    Object.assign(err, {
                        code: google_gax_1.grpc.status.UNKNOWN,
                        details: err.message,
        ...
        if (!(0, helper_1.isEmpty)(typeMap)) {
            Object.keys(typeMap).forEach(param => {
                ...
                if (process.env['SPANNER_ENABLE_UUID_AS_UNTYPED'] === 'true') {
                    const typeObject = codec_1.codec.createTypeObject(type);
                    if ((type.child &&
...
class Transaction extends Dml {
    ...
    commit(optionsOrCallback, cb) {
        const options = typeof optionsOrCallback === 'object' ? optionsOrCallback : {};
        const callback = typeof optionsOrCallback === 'function' ? optionsOrCallback : cb;
        let gaxOpts = 'gaxOptions' in options ? options.gaxOptions : options;
    ...
    rollback(gaxOptionsOrCallback, cb) {
        let gaxOpts = typeof gaxOptionsOrCallback === 'object' ? gaxOptionsOrCallback : {};
        const callback = typeof gaxOptionsOrCallback === 'function' ? gaxOptionsOrCallback : cb;
