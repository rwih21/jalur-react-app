...
exports.getGcpChannelFactoryClass = void 0;
...
function getGcpChannelFactoryClass(grpc) {
    ...
    return class GcpChannelFactory {
        ...
        options) {
            ...
            this.affinityKeyToChannelRef = new Map();
            this.affinityKeyLastAccessed = new Map();
            this.channelRefs = [];
            if (!options) {
                options = {};
            }
            ...
            if (gcpApiConfig) {
                if (gcpApiConfig.channelPool) {
                    ...
                    if (channelPool.minSize)
                        this.minSize = channelPool.minSize;
            ...
            this.cleanupTimer = setInterval(() => this.cleanupIdleKeys(), CLEANUP_INTERVAL_MS);
        ...
        getChannelRef(affinityKey) {
            ...
            for (let i = 1; i < this.channelRefs.length; i++) {
                if (this.channelRefs[i].getActiveStreamsCount() <
        ...
        cleanupIdleKeys() {
            ...
            const keysToUnbind = [];
            for (const [key, lastAccessed,] of this.affinityKeyLastAccessed.entries()) {
        ...
        shouldRequestDebugHeaders(lastRequested) {
            if (this.debugHeaderIntervalSecs < 0)
                ...
                return true;
            return (new Date().getTime() - lastRequested.getTime() >
         ...
         */
        bindIfUnbound(channelRef, affinityKey) {
            if (!affinityKey || !channelRef)
                return false;
        ...
        unbind(boundKey) {
            ...
            if (boundChannelRef) {
                ...
                if (this.affinityKeyLastAccessed.has(boundKey) ||
